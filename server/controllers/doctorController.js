...
exports.checkBookingAvailability = catchAsync(async (req, res, next) => {
  ...
  const fromTime = moment(req.body.time).subtract(30, "minutes").toDate();
  const toTime = moment(req.body.time).add(15, "minutes").toDate();
  ...
  const appointments = await Appointment.find({
    ...
    status: { $nin: ["rejected", "cancelled"] }, // Exclude rejected/cancelled appointments
  });
