...
const BookAppointment = () => {
  ...
  const [uploadDocuments, { isLoading: uploadLoading }] =
    useUploadDocumentsMutation();
  ...
  const appointmentHandler = async (appointmentData: AppointmentForm) => {
    ...
    if (appointment === "bookAppointment") {
      ...
      if (selectedFiles.length > 0 && uploadedDocuments.length === 0) {
        const formData = new FormData();
        selectedFiles.forEach((file) => {
          formData.append("documents", file);
        });

        const uploadResponse: any = await uploadDocuments(formData);
        if (uploadResponse?.data?.status) {
          ...
          setToast({
            ...toast,
            message:
              uploadResponse?.error?.data?.message ||
                        ...
                        <Form onKeyDown={onKeyDown}>
                          ...
                          {isAvailable && (
                            <Button
                              ...
                              variant="outlined"
                              fullWidth
                              disabled={appointmentLoading || uploadLoading}
                              sx={{
                                padding: "5px 30px",
                                textTransform: "capitalize",
                                margin: "0px 0 20px 0",
                            ...
                            >
                              {appointmentLoading || uploadLoading
                                ? "Booking..."
                                : "Book Appointment"}
                            </Button>
                          )}
              ...
              >
                ...
                <Box sx={{ marginTop: "10px" }}>
                  ...
                  {selectedFiles.length > 0 && (
                    <Box sx={{ marginTop: "10px", fontSize: "13px" }}>
